...
class profileController extends Controller
{
    ...
    function update(Request $request)
    {
        $request->validate([
            ...
            '_foto.mimes' => 'Foto yang dimasukkan hanya diperbolehkan berekstensi JPEG,JPG,PNG,GIF',
        ...
        if($request->hasFile('_foto')){
            ...
            metadata::updateOrCreate(['meta_key'=>'_foto'], 
            ['meta_value'=>$foto_baru]);
        }

        metadata::updateOrCreate(['meta_key'=>'_email'], 
        ['meta_value'=>$request->_email]);
        metadata::updateOrCreate(['meta_key'=>'_kota'], 
        ['meta_value'=>$request->_kota]);
        metadata::updateOrCreate(['meta_key'=>'_provinsi'], 
        ['meta_value'=>$request->_provinsi]);
        metadata::updateOrCreate(['meta_key'=>'_nohp'], 
        ['meta_value'=>$request->_nohp]);


        metadata::updateOrCreate(['meta_key'=>'_facebook'], 
        ['meta_value'=>$request->_facebook]);
        metadata::updateOrCreate(['meta_key'=>'_twitter'], 
        ['meta_value'=>$request->_twitter]);
        metadata::updateOrCreate(['meta_key'=>'_linkedln'], 
        ['meta_value'=>$request->_linkedln]);
        metadata::updateOrCreate(['meta_key'=>'_github'], 
